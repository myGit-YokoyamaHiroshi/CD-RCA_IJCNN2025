# -*- coding: utf-8 -*-
"""
Created on Tue Jun 18 09:23:54 2024
"""
import os
...
from my_modules.myfunctions import generate_VAR_data
...
fig_path = current_path + '/figures/comparison/z_%03.1f/'%z_mean
if os.path.exists(fig_path)==False:  # Make the directory for figures
    os.makedirs(fig_path)
    
save_path = current_path + '/save_data/z_%03.1f/'%z_mean
...
if os.path.exists(fullpath_save + '.npy') == False:
    ...
    for tri in range(Ntri):
        ...
        data_new   = generate_VAR_data(amat, Npcmci, Nx, 
                                       T_spurious = T_spurious, 
                                       confounder=True, Z=Z)
...
plt.show()
#%%
save_csv  = current_path + '/save_data/z_%03.1f/RCA_results.csv'%z_mean
col_lab   = ['$\phi_%d$'%(n+1) for n in range(Nnode)]
colmun    = np.hstack(('', col_lab, 'acc'))
line_att  = np.hstack(('CD-RCA(our)', np.mean(norm_att, axis=1),  np.array(acc_att)))
line_lime = np.hstack(('LIME',        np.mean(norm_lime, axis=1), np.array(acc_lime)))
line_eig  = np.hstack(('EIG',         np.mean(norm_eig, axis=1),  np.array(acc_eig)))
line_lc   = np.hstack(('LC',          np.mean(norm_lc, axis=1),   np.array(acc_lc)))
line_gpa  = np.hstack(('GPA',         np.mean(norm_gpa, axis=1),  np.array(acc_gpa)))
line_z    = np.hstack(('z-score',     np.mean(norm_z, axis=1),    np.array(acc_z)))
